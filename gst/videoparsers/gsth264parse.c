...
/* Modification: only packetized format(3gpp) need to be set passthrough */
#define CHECK_PACKETIZED_FOR_PASSTHROUGH
...
gst_h264_parse_process_nal (GstH264Parse * h264parse, GstH264NalUnit * nalu)
{
  ...
  if (h264parse->format == GST_H264_PARSE_FORMAT_AVC) {
    ...
    gboolean isnalu = FALSE;

    if (nalu->offset + nalu->size >= 3) {
    while (length < (nalu->offset + nalu->size - 3)) {
        if (nalu->data[length] == 0x00 &&
          nalu->data[length+1] == 0x00 &&
            nalu->data[length+2] == 0x01) {
          isnalu = TRUE;
          h264parse->format = GST_H264_PARSE_FORMAT_BYTE;
          GST_INFO_OBJECT(h264parse, "H264 format is NALU");
          break;
        } else {
          length++;
        }
...
#endif
    {
      GST_INFO_OBJECT (h264parse, "collecting NAL in AVC frame");
...
#ifdef GST_H264PARSE_MODIFICATION
  data[4] = 0xfc | ( h264parse->nal_length_size - 1);     /* nal_length_size_minus1                  */
#else
...
#ifdef GST_H264PARSE_MODIFICATION /* handle byte-stream codec_data */
    if(size >= 4) {
...
#ifdef CHECK_PACKETIZED_FOR_PASSTHROUGH
if(h264parse->packetized)
#endif /* CHECK_PACKETIZED_FOR_PASSTHROUGH */
 {
  {
    GstCaps *in_caps;
