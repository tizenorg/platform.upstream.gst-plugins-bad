...
gst_wayland_sink_check_use_gapless (GstWaylandSink * sink)
{
  ...
  if (sink->got_costum_event && sink->USE_TBM && sink->display->is_native_format)
    return TRUE;
...
gst_wayland_sink_event (GstBaseSink * bsink, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
      ...
      GST_LOG ("get GST_EVENT_CUSTOM_DOWNSTREAM EVENT: %s..state is %d", gst_structure_get_name (s), GST_STATE (sink));
	  GST_LOG ("CUSTOM EVENT: %s", gst_structure_get_name (s));
#if 1
...
gst_wayland_sink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
  ...
  if (sink->USE_TBM && sink->display->is_native_format) {
    ...
    GST_LOG_OBJECT (sink, "replace last_buffer: (%p) -> (%p)", sink->last_buffer, buffer);
    gst_buffer_replace (&sink->last_buffer, buffer);

	GST_LOG_OBJECT (sink, "after gst_buffer_replace last_buffer %p, ref_count(%d)", sink->last_buffer,
		GST_OBJECT_REFCOUNT_VALUE (sink->last_buffer));

	GST_LOG_OBJECT (sink, "after gst_buffer_replace buffer %p, ref_count(%d)", buffer,
		GST_OBJECT_REFCOUNT_VALUE (buffer));
