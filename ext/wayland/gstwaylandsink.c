...
static int
gst_wayland_sink_add_mm_video_buf_info_to_flush (GstWlDisplay * display, MMVideoBuffer * mm_video_buf)
{
  FUNCTION;
  g_return_val_if_fail (display != NULL, FALSE);
  g_return_val_if_fail (mm_video_buf != NULL, FALSE);
...
static void
gst_wayland_sink_get_mm_video_buf_info (GstWlDisplay * display, MMVideoBuffer * mm_video_buf)
{
  FUNCTION;
...
gst_wayland_sink_get_mm_video_buf_info(GstWlDisplay * display, GstBuffer * buffer)
{
  ...
  if (mm_video_buf->type == MM_VIDEO_BUFFER_TYPE_TBM_BO) {
    ...
    if (display->flush_request) {
      if(!gst_wayland_sink_add_mm_video_buf_info_to_flush(display, mm_video_buf)){
...
#endif
      /* normal routine */
      gst_wayland_sink_get_mm_video_buf_info(display, mm_video_buf);
  } else {
