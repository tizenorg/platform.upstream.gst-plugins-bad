...
#ifdef GST_WLSINK_ENHANCEMENT
static void gst_wayland_sink_update_property_value (GstWaylandSink * sink);
#endif
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_FLIP:
      ...
      if (sink->flip) {
        gst_wl_window_set_flip (sink->window, sink->orientation);
      }
      sink->video_info_changed = TRUE;
      if (GST_STATE (sink) == GST_STATE_PAUSED) {
        /*need to render current buffer*/
...
#ifdef GST_WLSINK_ENHANCEMENT
    gst_wayland_sink_update_property_value (sink);
    sink->video_info_changed = TRUE;
#else
...
static void
gst_wayland_sink_update_property_value (GstWaylandSink * sink)
{
  FUNCTION_ENTER ();
