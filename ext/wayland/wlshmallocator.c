    ...
    GstAllocationParams * params)
{
  ...
  if (self->display->tbm_bo_c_idx == self->display->tbm_bo_max_idx)
    ...
    const GstVideoInfo * info)
{
  ...
  if (display->is_native_format == TRUE) {
    ...
    GST_DEBUG ("ts_info.width(%d) height(%d) format(%d) bpp(%d) num_planes(%d)",
        ts_info.width, ts_info.height, gst_wl_tbm_format_to_string (format), ts_info.bpp, ts_info.num_planes);
    ...
    int idx;
	if (display->tbm_bo_u_idx == display->tbm_bo_max_idx)
  ...
  GST_ERROR ("wayland_tbm_client_create_buffer create wl_buffer %p", wbuffer);
