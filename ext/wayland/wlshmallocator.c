    ...
    GstAllocationParams * params)
{
  ...
  if (self->display->USE_TBM) {
	...
	if (self->display->tbm_need_limit_idx && self->display->tbm_bo_c_idx == self->display->tbm_bo_max_idx)
...
//	  }
	...
	wbuffer = wl_shm_pool_create_buffer (wl_pool, 0, width, height, stride, format);
