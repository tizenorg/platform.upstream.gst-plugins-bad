    ...
    GstVideoInfo * info)
{
  ...
  *((guint32 *) mapinfo.data) = 0;      /* paint it black */
  ...
  GST_INFO ("wl_surface_damage_buffer (%p, %d, %d, %d, %d)",
...
#endif
{
  ...
  if (display->need_shell_surface) {
...
#ifdef GST_WLSINK_ENHANCEMENT
    GST_LOG ("call tizen_policy_get_subsurface");
    if (display->wl_surface_id && parent == NULL) {
      ...
      GST_INFO
          ("area_subsurface (%p) = tizen_policy_get_subsurface(%p, %p, %d)",
      ...
      GST_INFO
          ("area_subsurface (%p) = wl_subcompositor_get_subsurface(%p, %p, %p)",
  ...
  GST_INFO
      ("video_subsurface (%p) = wl_subcompositor_get_subsurface(%p, %p, %p)",
  ...
  GST_INFO ("tizen_area_viewport(%p) = tizen_video_get_viewport(%p, %p)",
  ...
  GST_INFO ("tizen_video_viewport(%p) = tizen_video_get_viewport(%p, %p)",
    ...
    gboolean commit)
{
  ...
  switch (window->disp_geo_method) {
    ...
    case DISP_GEO_METHOD_CUSTOM_ROI:
      GST_LOG ("DISP_GEO_METHOD_CUSTOM_ROI");
      break;
    default:
      break;
  }
  if (tizen_disp_mode > -1) {
    ...
    GST_INFO ("tizen_destination_mode_set (%p, %d)", tizen_dest_mode,
        tizen_disp_mode);
    ...
    const GstVideoInfo * info)
{
  ...
  if (window->buffer_width != window->display->buffer_width
    ...
    window->buffer_height = window->display->buffer_height;
    GST_INFO ("tizen_viewport_set_destination (%p, %d, %d, %d,%d)",
...
gst_wl_window_set_rotate_angle (GstWlWindow * window, guint rotate_angle)
{
  ...
  GST_LOG ("rotate_angle value is (%d)", window->rotate_angle);
}
...
gst_wl_window_set_disp_geo_method (GstWlWindow * window, guint disp_geo_method)
{
  ...
  GST_LOG ("disp_geo_method value is (%d)", window->disp_geo_method);
}
...
gst_wl_window_set_orientation (GstWlWindow * window, guint orientation)
{
  ...
  GST_LOG ("orientation value is (%d)", window->orientation);
}
...
gst_wl_window_set_flip (GstWlWindow * window, guint flip)
{
  ...
  g_return_if_fail (window != NULL);
  window->flip = flip;
  GST_LOG ("flip value is (%d)", window->flip);
}
