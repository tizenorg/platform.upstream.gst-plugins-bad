...
window->video_object = tizen_video_get_object(display->tizen_video, window->video_surface); 
...
#if 1  /* not create shell_surface here for enlightenment */
    ...
    struct wl_surface * parent)
{
  ...
  if (display->tizen_policy)
    tizen_policy_place_subsurface_below_parent (display->tizen_policy, window->area_subsurface);
...
#ifdef GST_WLSINK_ENHANCEMENT
	if(window->render_rectangle.x == x && window->render_rectangle.y == y && window->render_rectangle.w == w && window->render_rectangle.h == h) {
