...
gst_wl_window_new_internal (GstWlDisplay * display, struct wl_surface * parent)
{
  ...
  if (display->use_parent_wl_surface && parent != NULL) {
    window->area_subsurface = wl_subcompositor_get_subsurface (display->subcompositor, window->area_surface, parent);
    ...
    const GstVideoInfo * info)
{
  ...
  if (G_LIKELY (buffer)) {
  	GST_ERROR ("buffer (%p)", buffer);
    gst_wl_buffer_attach (buffer, window->video_surface);
  } else {
    GST_ERROR ("GstWlBuffer (%p)", buffer);
